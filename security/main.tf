...
resource "ibm_is_security_group" "internal_security_group" {
  name           = "${var.name}-wg-internal-group"
  vpc            = var.vpc
  resource_group = var.resource_group
}


resource "ibm_is_security_group_rule" "internal_ssh_from_bastion" {
  group     = ibm_is_security_group.internal_security_group.id
  direction = "inbound"
  remote    = var.bastion_security_group
  tcp {
    port_min = 22
...
resource "ibm_is_security_group_rule" "internal_ping" {
  group     = ibm_is_security_group.internal_security_group.id
  direction = "inbound"
  remote    = "0.0.0.0/0"
  icmp {
...
# from https://cloud.ibm.com/docs/vpc?topic=vpc-service-endpoints-for-vpc
resource "ibm_is_security_group_rule" "internal_cse_dns_1" {
  group     = ibm_is_security_group.internal_security_group.id
  direction = "outbound"
  remote    = "161.26.0.10"
  udp {
...
resource "ibm_is_security_group_rule" "internal_cse_dns_2" {
  group     = ibm_is_security_group.internal_security_group.id
  direction = "outbound"
  remote    = "161.26.0.11"
  udp {
...
resource "ibm_is_security_group_rule" "internal_private_dns_1" {
  group     = ibm_is_security_group.internal_security_group.id
  direction = "outbound"
  remote    = "161.26.0.7"
  udp {
...
resource "ibm_is_security_group_rule" "internal_private_dns_2" {
  group     = ibm_is_security_group.internal_security_group.id
  direction = "outbound"
  remote    = "161.26.0.8"
  udp {
...
resource "ibm_is_security_group_rule" "internal_all_out" {
  group     = ibm_is_security_group.internal_security_group.id
  direction = "outbound"
  remote    = "0.0.0.0/0"
}